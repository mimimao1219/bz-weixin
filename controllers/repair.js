...
exports.list = function (req, res, next) {
	...
	var ep1 = EventProxy.create('query', function (query) {
		...
		if (usertype === '2') {
			...
			//var query4 = _.merge({ statu: 1 }, query);
			RepairCurrentModel.find(query1, {}, options, ep.done('repairCurrents'));
			//RepairCurrentModel.find(query4, {}, options, ep.done('repairCurrents1'));
			RepairCurrentModel.find(query2, {}, options, ep.done('repairfinishs'));
	...
	if (usertype === '2') {
		RepairManagerModel.find({ managerid: req.session.user.UserId }, 'repairtype',  function (err, repairmanagers) {
			if (err) {
				return callback(err);
			}
